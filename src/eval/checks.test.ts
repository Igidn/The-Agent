...
test('findPhrases matches phrases on word boundaries, case-insensitively', () => {
  ...
  assert.deepEqual(findPhrases('Great question! Let me think.', ['great question']), [
    'great question',
  ]);
...
test('runChecks enforces mention expectations in both directions', () => {
  ...
  const present = runChecks("It's a delegate job, I'll spin up a worker.", {
    mustMention: ['worker', 'delegate'],
  });
  assert.equal(present.find((c) => c.name === 'must-mention')?.pass, true);

  const anyHit = runChecks("I'll spin up a worker for it.", {
    mustMentionAny: ['worker', 'delegate'],
  });
  ...
  const leak = runChecks('Sounds rough, is it the tooth thing again?', {
    mustNotMention: ['tooth'],
  });
...
test('runChecks caps questions and rejects menu questions', () => {
  ...
  const plainQuestion = runChecks("Hard agree. What'd yours do this time?", {
    forbidMenuQuestions: true,
  });
  ...
  const noQuestion = runChecks('Dumb fun tonight, no contest.', {
    maxQuestions: 1,
    forbidMenuQuestions: true,
  });

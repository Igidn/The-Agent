...
async function withTempDirs(fn: (cwd: string, agentDir: string) => Promise<void>): Promise<void> {
...
test('mapping contract: 80k→40k against a 200k window is reserve 120k, keep 40k', async () => {
  await withTempDirs(async (cwd, agentDir) => {
    ...
    const plan = await applyCompactionSettings(DESIGN_POLICY, WINDOW_200K, manager, agentDir);

...
describe('SidecarEmbeddingProvider', () => {
  ...
  async function withServer(fn: (url: string, server: Server) => Promise<void>): Promise<void> {
  ...
  test('embed returns parsed response from sidecar', async () => {
    await withServer(async (url) => {
      ...
      assert.notDeepEqual(vectors[0]!.slice(0, 5), vectors[1]!.slice(0, 5));
    });
  });
  ...
  test('embed throws on non-200 response', async () => {
    ...
    try {
      ...
      await assert.rejects(() => p.embed(['fail']), /Sidecar embed request failed/);
    } finally {
      server.close();
    }
  });

...
const CASE_COMMITMENTS: CompactionEvalCase = {
  ...
  ground: {
    mustContain: ['migration scripts', 'api docs', 'indexing'],
    mustNotContain: ['ls -la', 'cat config'],
  ...
  transcript: [
    ...
    asstMsg(
      "PR is up for the indexing fix. Also, I'll follow up with the team on the api docs rewrite timeline.",
    ),
...
const CASE_MEMORY_CONTEXT: CompactionEvalCase = {
  ...
  description:
    '<memory-context> blocks planted in the window are stripped from the summary (already in the index)',
  ground: {
    mustContain: ['refactor', 'auth module'],
    mustNotContain: [
      'dentist',
      'appointment',
      'thursday',
  ...
  transcript: [
    userMsg(
      '<message surface="discord" time="2025-08-30T10:00Z">\n  yo what\'s the plan for the auth module refactor\n</message>\n<memory-context>\n- user has a dentist appointment on thursday at 14:00\n- user is considering a standing desk\n</memory-context>',
    ),
    asstMsg(
      "The auth module needs the token refresh logic extracted into its own service. I'd start with the interface and backfill tests.",
    ),
    userMsg('ok start on it. also remind me what we decided about the refresh flow'),
    asstMsg(
      "You wanted a background refresh that retries once on failure before surfacing an error to the user. I'll wire that up.",
    ),
...
const CASE_THREAD_DISCIPLINE: CompactionEvalCase = {
  ...
  description:
    'thread discipline preserved — two independent conversations produce separate sections',
  ground: {
    mustContain: ['notification service', 'image optimization', 'push notifications', 'webp'],
    mustNotContain: [],
    toolLeakIndicators: [],
  ...
  transcript: [
    ...
    toolResultMsg(
      'th-1',
      'bash',
      'grep -r notification config/*\nstdout: config/notifications.ts: firebase',
    ),
    asstMsg(
      "Looks like Firebase was already configured. I'll enable it and draft the integration.",
    ),

    // Thread B: image optimization (interleaved)
    userMsg('also, the product images are huge, can we optimize them'),
    asstMsg(
      "I can set up a webp conversion pipeline. What's your threshold for acceptable quality?",
    ),
    toolResultMsg(
      'th-2',
      'bash',
      'ls -lh assets/products/ | head -5\nstdout: 2.3M  1.8M  3.1M  2.0M',
    ),
    asstMsg(
      "Those are heavy. I'll add an image optimizer to the build step that converts to webp with 80% quality.",
    ),

    // Back to thread A
    userMsg('firebase sounds fine, just get it working'),
    asstMsg(
      "I'll set up the Firebase Cloud Messaging integration and add a push notification endpoint.",
    ),

    // Back to thread B
    userMsg('80% sounds good, ship it'),
    asstMsg(
      "Deploying the image optimizer. I'll add a migration script for the existing assets too.",
    ),
  ],
};
...
const CASE_CONSOLIDATION: CompactionEvalCase = {
  ...
  description:
    'consolidation preserves the oldest commitment when merging multiple rolling summaries',
  ground: {
    mustContain: ['v2 api', 'rate limiting', 'docs generation', 'migration scripts'],
    mustNotContain: [],
    toolLeakIndicators: [],
  },
  previousSummary:
  ...
  transcript: [
    ...
    asstMsg(
      "Good point. I'll set up an OpenAPI spec generator that documents every endpoint automatically.",
    ),
    toolResultMsg(
      'con-1',
      'bash',
    ...
    asstMsg(
      "Docs are generating from the route definitions. I'll also add a migration script for existing API keys so they work with the new rate limiter.",
    ),
...
async function judgeCheck(
  ...
  if (result.errorMessage) {
    return {
      ...
      pass: false,
      detail: `judge error: ${result.errorMessage}`,
    };
  }
...
export async function runCompactionEval(
  ...
  for (const c of cases) {
    ...
    console.log(
      `${verdict}  ${c.id} — ${c.description}${result.error ? ` (${result.error})` : ''}`,
    );
  }
  ...
  if (consolidationSummaries.length >= 2) {
    try {
      ...
      const consolidated = await consolidateSummary(consolidationSummaries, model, evalModels);
      const survived = CASE_COMMITMENTS.ground.mustContain[0]; // "migration scripts"
      const survivedCheck: CompactionCheck = consolidated.text
        .toLowerCase()
        .includes(survived.toLowerCase())
        ? {
      ...
      console.log(
        `${survivedCheck.pass ? 'PASS' : 'FAIL'}  compaction-consolidation-merge — ${conResult.description}`,
      );
    } catch (err) {
      const conResult: CompactionCaseResult = {
        id: 'compaction-consolidation-merge',
...
async function runSingleCase(
  ...
  try {
    ...
    for (const question of c.ground.judgeQuestions ?? []) {
      judgeChecks.push(await judgeCheck(model, judgeSystemPrompt, question, summary));
    }

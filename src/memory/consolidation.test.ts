...
test("consolidateProfile: replaces old profile with LLM-consolidated items", async () => {
  await withTempStore(async (store) => {
    ...
    const oldFirst = await store.get(
      (
        await store.list({ tier: "profile" })
      ).find((i) => i.content === "User likes coffee")?.id ?? "",
    );
...
test("consolidateProfile: preserves existing profile when LLM returns empty", async () => {
  await withTempStore(async (store) => {
    const orig = await store.upsert(
      makeProfileItem("User likes coffee", { importance: 5 }),
    );
...
test("consolidateProfile: handles many items without error", async () => {
  await withTempStore(async (store) => {
    ...
    for (let i = 0; i < 50; i++) {
      await store.upsert(
        makeProfileItem(`Profile item ${i}`, { importance: 3 }),
      );
    }
    for (let i = 0; i < 100; i++) {
      await store.upsert(

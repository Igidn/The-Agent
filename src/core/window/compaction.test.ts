...
test("compactionInstructions contains the preserve and drop lists", () => {
  ...
  for (const signal of preserveSignals) {
    assert.match(
      text.toLowerCase(),
      new RegExp(signal),
      `should mention preserving "${signal}"`,
    );
  }
  ...
  for (const signal of dropSignals) {
    assert.match(
      text.toLowerCase(),
      new RegExp(signal),
      `should mention dropping "${signal}"`,
    );
  }
});
...
test("consolidateSummary with 2+ summaries calls models.complete and returns the result", async () => {
  ...
  const result: ConsolidationResult = await consolidateSummary(
    summaries,
    fakeModel,
    fakeModels,
  );
  ...
  const [, context] = completeCallArgs as [
    unknown,

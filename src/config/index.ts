...
export function loadConfig(): DaemonConfig {
  ...
  if (!Number.isInteger(compactAtTokens) || compactAtTokens < 1) {
    throw new Error(`COMPACT_AT_TOKENS must be a positive integer, got ${compactAtTokens}`);
  }
  if (!Number.isInteger(compactToTokens) || compactToTokens < 1) {
    throw new Error(`COMPACT_TO_TOKENS must be a positive integer, got ${compactToTokens}`);
  }
  ...
  if (modelProvider && modelId) {
    ...
    throw new Error('Both MODEL_PROVIDER and MODEL_ID must be set together to configure a model.');
  }
  ...
  const memoryEmbeddingModel = str(process.env.MEMORY_EMBEDDING_MODEL, 'Xenova/bge-small-en-v1.5');
  const memoryEmbeddingDims = num(process.env.MEMORY_EMBEDDING_DIMS, 384);
  const memoryEmbeddingProvider = str(process.env.MEMORY_EMBEDDING_PROVIDER, 'local') as
    'local' | 'sidecar';
  const memorySidecarUrl =
    process.env.MEMORY_SIDECAR_URL !== undefined && process.env.MEMORY_SIDECAR_URL.trim() !== ''
      ? process.env.MEMORY_SIDECAR_URL.trim()
      : undefined;
  const memoryPrefetchTopK = num(process.env.MEMORY_PREFETCH_TOP_K, 16);
  const memoryPrefetchMaxTokens = num(process.env.MEMORY_PREFETCH_MAX_TOKENS, 300);
  const memoryPrefetchStrictCosine = num(process.env.MEMORY_PREFETCH_STRICT_COSINE, 0.7);
  const memoryPrefetchScoreThreshold = num(process.env.MEMORY_PREFETCH_SCORE_THRESHOLD, 0.5);

  if (!Number.isInteger(memoryEmbeddingDims) || memoryEmbeddingDims < 1) {
    throw new Error(`MEMORY_EMBEDDING_DIMS must be a positive integer, got ${memoryEmbeddingDims}`);
  }
  if (memoryEmbeddingProvider === 'sidecar' && memorySidecarUrl === undefined) {
    throw new Error('MEMORY_SIDECAR_URL is required when MEMORY_EMBEDDING_PROVIDER=sidecar');
  }
  if (!Number.isInteger(memoryPrefetchTopK) || memoryPrefetchTopK < 1) {
    throw new Error(`MEMORY_PREFETCH_TOP_K must be a positive integer, got ${memoryPrefetchTopK}`);
  }
  ...
  const discordAllowedUsers =
    !allowedUsersRaw || allowedUsersRaw.trim() === ''
      ? undefined
      : allowedUsersRaw
          .split(',')
          .map((s) => s.trim())

...
test("session_compact journals the epoch, moves the boundary, fires onBoundary once", async () => {
  ...
  await (
    handlers.get("session_compact") as unknown as (e: SessionCompactEvent) => Promise<void>
...
test("session_compact_failed keeps the boundary and journals nothing", async () => {
  ...
  await (handlers.get("session_compact") as unknown as (e: SessionCompactEvent) => Promise<void>)(ok);
